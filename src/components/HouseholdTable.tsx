...
const HouseholdTable: React.FC<HouseholdTableProps> = ({ households, onEdit, onDelete, hasActiveFilters, sortConfig, onSort }) => {
  ...
  const getEmptyStateMessage = () => {
    if (hasActiveFilters) {
      return (
        <span>
          Không tìm thấy hộ gia đình nào phù hợp với tiêu chí lọc.
        </span>
      );
    }
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-green-100">
          <tr>
            ...
            <th scope="col" className="px-4 py-3 text-left text-xs uppercase tracking-wider w-12">
              <SortableHeader title="STT" sortKey="stt" />
            </th>
            <th scope="col" className="px-6 py-3 text-left text-xs uppercase tracking-wider">
              <SortableHeader title="Chủ hộ" sortKey="headOfHouseholdName" />
            </th>
            <th scope="col" className="px-6 py-3 text-left text-xs uppercase tracking-wider">
              <SortableHeader title="Số căn" sortKey="apartmentNumber" />
            </th>
            <th scope="col" className="px-6 py-3 text-left text-xs font-bold text-green-800 uppercase tracking-wider">Thành viên</th>
            <th scope="col" className="px-6 py-3 text-left text-xs uppercase tracking-wider">
              <SortableHeader title="SĐT" sortKey="phone" />
            </th>
              ...
              <React.Fragment key={household.id}>
                <tr className="hover:bg-gray-50 transition-colors duration-150">
                  ...
                  <td className="px-6 py-4 align-middle">
                    <span className="px-2.5 py-1 text-sm font-semibold rounded-full bg-gray-200 text-gray-700">
                  ...
                  <td className="px-6 py-4 align-middle">
                    <div className="flex items-center space-x-3">
                      <button onClick={() => onEdit(household)} className="text-blue-600 hover:text-blue-900 transition-colors duration-150" title="Chỉnh sửa">
                        <PencilIcon />
                      </button>
                      <button onClick={() => onDelete(household.id)} className="text-red-600 hover:text-red-900 transition-colors duration-150" title="Xóa">
                        <TrashIcon />
                      </button>
                    </div>
                  </td>
