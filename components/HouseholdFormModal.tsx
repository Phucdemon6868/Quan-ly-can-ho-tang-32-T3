...
const HouseholdFormModal: React.FC<HouseholdFormModalProps> = ({ isOpen, onClose, onSave, householdToEdit }) => {
  ...
  const handleMemberChange = (memberId: string, field: keyof Member, value: string | Gender | Relationship) => {
  ...
  return (
    ...
    >
      <div
        ...
        className={`fixed inset-0 sm:inset-auto sm:top-1/2 sm:left-1/2 bg-white sm:rounded-lg shadow-xl w-full sm:w-full sm:max-w-3xl flex flex-col transform transition-all duration-300 ease-in-out sm:max-h-[90vh] ${
          isOpen
            ? 'translate-y-0 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:scale-100 opacity-100'
            : 'translate-y-full sm:-translate-x-1/2 sm:-translate-y-1/2 sm:scale-95 opacity-0'
        }`}
      >
        {/* Header */}
        <div className="flex-shrink-0 p-4 md:p-6 border-b flex justify-between items-center">
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
          ...
          {/* Footer */}
          <div className="flex-shrink-0 p-4 md:p-6 bg-gray-50 border-t flex flex-col sm:flex-row justify-end gap-3">
